      ...
      switch (cmd.toUpperCase()) { // Convierte el comando a mayúsculas para que sea insensible a mayúsculas y minúsculas
        ...
        case 'PUT': { // Recibe un archivo del cliente
          ...
          socket.on('data', chunk => {
            // Verificamos si hemos recibido el delimitador EOF
            const chunkStr = chunk.toString();
            if (chunkStr.includes('---EOF---')) {
              // Separamos el contenido del archivo del delimitador
              const eofIndex = chunk.indexOf('\n---EOF---\n');
              if (eofIndex !== -1) {
                // Solo tomamos la parte antes del EOF
                const actualFileData = chunk.slice(0, eofIndex);
                fileData = Buffer.concat([fileData, actualFileData]);
                eofReceived = true;
                
                // Procesamos el archivo inmediatamente
              ...
              // Solo agregamos datos si no hemos recibido EOF
