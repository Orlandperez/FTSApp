...
socket.on('data', chunk => {
  ...
  if (!readyReceived && message.trim() === 'READY: PUT') {
    ...
    // Enviamos un delimitador especial para indicar fin de archivo
    socket.write('\n---EOF---\n');
    console.log('[PUT] Archivo y delimitador enviados, esperando confirmación...');
  }
  
  // Si recibimos la confirmación, cerramos
  if (readyReceived && message.includes('OK: PUT complete')) {
    console.log('[PUT] Confirmación recibida, cerrando conexión...');
    socket.end();
  }
});
