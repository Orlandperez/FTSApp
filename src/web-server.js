...
app.post('/api/files', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    // Solo intentamos borrar si el archivo aún existe
    fs.access(tempPath, fs.constants.F_OK, (accessErr) => {
      if (!accessErr) {
        // El archivo existe, lo eliminamos
        fs.unlink(tempPath, (unlinkErr) => {
          if (unlinkErr) {
            console.warn("Error al eliminar archivo temporal de 'uploads':", tempPath, unlinkErr.message);
          } else {
            console.log("Archivo temporal eliminado correctamente:", tempPath);
          }
        });
      }
      // Si el archivo no existe (accessErr), no hacemos nada - ya fue eliminado
    });
  }
});
